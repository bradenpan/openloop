...
async def _stream_agent_response(conversation_id: str, message: str) -> None:
    """Background task: send message to session manager and publish events to event bus.
...
def close_conversation(conversation_id: str, db: Session = Depends(get_db)) -> ConversationResponse:
...
def get_summaries(conversation_id: str, db: Session = Depends(get_db)) -> list[SummaryResponse]:
